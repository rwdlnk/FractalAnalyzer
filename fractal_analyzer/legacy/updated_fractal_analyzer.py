...
# ================ Plotting Functions ================
    def plot_results_separate(self, segments, box_sizes, box_counts, fractal_dimension,
        ...
        """Creates separate plots with format determined by eps_plots flag."""
        import matplotlib

        # Get segment count and plot settings
        segment_count = len(segments)
        settings = self._get_plot_settings(segment_count)

        # Configure matplotlib based on settings
        if not self.eps_plots and segment_count > 20000:
            # Only configure chunking for PNG with large datasets
            matplotlib.rcParams['agg.path.chunksize'] = min(100000, max(20000, segment_count // 5))
        ...
        print(f"Plotting {segment_count} curve segments...")

        # Plotting strategy based on format and complexity
        if self.eps_plots:
            # EPS: Always use full quality, no sampling
            x_points = []
            y_points = []
            ...
            plt.plot(x_points, y_points, 'k-', linewidth=1, rasterized=False)
            print(f"EPS: Plotted all {segment_count} segments without sampling")
            
        else:
            # PNG: Use adaptive sampling for large datasets
            if segment_count > 50000:
                step = max(1, segment_count // 10000)
                sampled_segments = segments[::step]
                print(f"PNG: Sampled {len(sampled_segments)} segments for visualization")
            else:
            ...
            plt.plot(x_points, y_points, 'k-', linewidth=1, 
                    rasterized=settings.get('rasterized', False))
        ...
        # Generate filename
        if custom_filename:
            filename_base = os.path.splitext(custom_filename)[0]  # Remove extension
        else:
            filename_base = f'{self.fractal_type if self.fractal_type else "fractal"}_curve'
            if plot_boxes:
                filename_base += '_with_boxes'
            if level is not None:
                filename_base += f'_Level_{level}'

        # Save with appropriate format
        try:
            saved_file = self._save_plot(filename_base, segment_count=segment_count)
            print(f"Curve plot saved as: {saved_file}")
        except Exception as e:
            print(f"Error saving curve plot: {str(e)}")
    ...
    def _plot_loglog(self, box_sizes, box_counts, fractal_dimension, error, intercept=None,
                    custom_filename=None):
        """Plot ln-ln analysis with format determined by eps_plots flag."""
        ...
        def scientific_formatter(x, pos):
            ...
            if abs(coef - 1.0) < 0.01:
                return r'$10^{%d}import numpy as np
...
class FractalAnalyzer:
    ...
    def __init__(self, fractal_type: Optional[str] = None, no_titles: bool = False, eps_plots: bool = False):
        ...
        Args:
            ...
            eps_plots: If True, generate EPS format plots for publication
        """
    ...
    def _configure_matplotlib_for_eps(self):
        """Configure matplotlib for high-quality EPS output."""
        import matplotlib
        
        # EPS-specific configuration
        matplotlib.rcParams['ps.useafm'] = True  # Use Adobe Font Metrics
        matplotlib.rcParams['pdf.use14corefonts'] = True  # Use core fonts
        matplotlib.rcParams['text.usetex'] = False  # Don't require LaTeX for compatibility
        ...
        print("Configured matplotlib for high-quality EPS output")
    
    def _get_plot_settings(self, segment_count=None):
        """Get appropriate plot settings based on output format and complexity."""
        if self.eps_plots:
            # EPS settings - always high quality
            return {
                'format': 'eps',
                'dpi': 300,  # EPS is vector, but this affects rasterized elements
                'figsize': (10, 8),
                'bbox_inches': 'tight',
                'facecolor': 'white',
                'edgecolor': 'none',
                'rasterized': False,  # Keep vector format
                'extension': '.eps'
            }
        else:
            # PNG settings - adaptive based on complexity
            if segment_count and segment_count > 50000:
                settings = {
                    'format': 'png',
                    'dpi': 150,
                    'figsize': (14, 12),
            ...
            elif segment_count and segment_count > 20000:
                settings = {
                    'format': 'png', 
                    'dpi': 200,
                    'figsize': (12, 10),
    ...
    def _save_plot(self, filename_base, **kwargs):
        """Save plot with appropriate format and settings."""
        settings = self._get_plot_settings(kwargs.get('segment_count'))
        
        # Remove segment_count from kwargs if present
        kwargs.pop('segment_count', None)
        
        # Construct filename with appropriate extension
        filename = filename_base + settings['extension']
        
        # Merge provided kwargs with default settings
        save_kwargs = {k: v for k, v in settings.items() 
                      if k not in ['extension', 'figsize', 'rasterized']}
        save_kwargs.update(kwargs)
        
        try:
            print(f"Saving plot to {filename} in {settings['format'].upper()} format...")
            ...
            if settings['format'] == 'eps':
                # Fallback to PNG if EPS fails
                print("Attempting fallback to PNG format...")
                fallback_name = filename_base + '.png'
                plt.savefig(fallback_name, format='png', dpi=300, bbox_inches='tight')
                print(f"Saved fallback PNG: {fallback_name}")
    ...
    def analyze_linear_region(self, segments, fractal_type=None, plot_results=True,
        ...
        if theoretical_dimension is not None:
            ...
            for i, (window, dim, r2) in enumerate(zip(windows, dimensions, r_squared)):
                ...
                return r'$3{\times}10^{%d}import numpy as np
import matplotlib.pyplot as plt
from matplotlib.patches import Rectangle
from matplotlib.collections import PatchCollection
from scipy import stats
    ...
    def analyze_linear_region(self, segments, fractal_type=None, plot_results=True,
        ...
        if theoretical_dimension is not None:
            ...
            for i, (window, dim, r2) in enumerate(zip(windows, dimensions, r_squared)):
                ...
                return r'${%.1f}{\times}10^{%d}import numpy as np
import matplotlib.pyplot as plt
from matplotlib.patches import Rectangle
from matplotlib.collections import PatchCollection
from scipy import stats
...
# ================ Advanced Analysis Functions ================
    def analyze_linear_region(self, segments, fractal_type=None, plot_results=True,
        ...
        if custom_filename:
            ...
            filename_base = 'box_counting_loglog'
            if self.fractal_type:
                filename_base = f'{self.fractal_type}_box_counting_loglog'

        # Save with appropriate format
        try:
            saved_file = self._save_plot(filename_base)
            print(f"Log-log plot saved as: {saved_file}")
        except Exception as e:
            print(f"Error saving log-log plot: {str(e)}")
    ...
    def _plot_linear_region_analysis(self, windows, dimensions, errors, r_squared,
        ...
        filename_base = 'linear_region_analysis'
        if fractal_type:
            filename_base = f'{fractal_type}_linear_region_analysis'
            
        try:
            saved_file = self._save_plot(filename_base)
            print(f"Linear region analysis saved as: {saved_file}")
        except Exception as e:
            print(f"Error saving linear region analysis: {str(e)}")
            
        plt.close()

    def _plot_dimension_vs_level(self, levels, dimensions, errors, r_squared,
                               theoretical_dimension, fractal_type):
        """Plot dimension vs. iteration level with format determined by eps_plots flag."""
        settings = self._get_plot_settings()
        ...
        # Save with appropriate format
        filename_base = f'{fractal_type}_dimension_vs_level'
        try:
            saved_file = self._save_plot(filename_base)
            print(f"Dimension vs level plot saved as: {saved_file}")
        except Exception as e:
            print(f"Error saving dimension vs level plot: {str(e)}")
    ...
    def _plot_window_analysis_separate(self, windows, dimensions, errors, r_squared,
                                     optimal_window, optimal_dimension, theoretical_dimension,
                                     fractal_type=None):
        """Plot sliding window analysis with format determined by eps_plots flag."""
        settings = self._get_plot_settings()
        
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=settings['figsize'], sharex=True)
        ...
        filename_base = 'sliding_window_analysis'
        if fractal_type:
            filename_base = f'{fractal_type}_sliding_window_analysis'
            
        try:
            saved_file = self._save_plot(filename_base)
            print(f"Sliding window analysis saved as: {saved_file}")
        except Exception as e:
            print(f"Error saving sliding window analysis: {str(e)}")
...
# ================ Advanced Analysis Functions ================
    def analyze_linear_region(self, segments, fractal_type=None, plot_results=True,
        ...
        # Generate filename and save
        filename_base = 'box_counting_with_optimal_region'
        if fractal_type:
            filename_base = f'{fractal_type}_box_counting_with_optimal_region'
            
        try:
            saved_file = self._save_plot(filename_base)
            print(f"Log-log plot with optimal region saved as: {saved_file}")
        except Exception as e:
            print(f"Error saving log-log plot with optimal region: {str(e)}")
...
Examples:
  ...
  # Generate EPS plots for publication
  python fractal_analyzer.py --generate koch --level 7 --eps_plots --no_titles
  ...
  # Run both analyses in sequence with EPS output
  python fractal_analyzer.py --generate dragon --analyze_linear_region --analyze_iterations --eps_plots --no_titles
""")
    ...
    parser.add_argument('--eps_plots', action='store_true',
                       help='Generate EPS format plots for publication (vector graphics, high quality)')
    ...
    if args.eps_plots:
        print(f"Plot format: EPS (vector graphics for publication)")
        print(f"Plot quality: Maximum (no sampling, embedded fonts)")
    else:
        print(f"Plot format: PNG (raster graphics)")
        print(f"Plot quality: Adaptive based on complexity")
