...
def analyze_single_timestep(args):
    ...
    try:
        ...
        # Create analyzer for this specific timestep with CONREC support
        analyzer = RTAnalyzer(
            timestep_output_dir, 
        ...
        result['extraction_method'] = 'CONREC' if analysis_params.get('use_conrec', False) else 'scikit-image'  # NEW
        ...
        return {
            ...
            'extraction_method': 'CONREC' if analysis_params.get('use_conrec', False) else 'scikit-image',  # NEW
            'error': str(e),
            'status': 'failed'
        }
...
def analyze_parallel_temporal_evolution(data_dir, resolution, output_dir=None, 
                                       ...
                                       use_conrec=False):  # NEW: CONREC parameter
    """
    ...
    Args:
        ...
        use_conrec: Use CONREC precision interface extraction (NEW)
    ...
    if output_dir is None:
        method_suffix = "_conrec" if use_conrec else "_skimage"  # NEW
    ...
    print(f"Interface extraction: {'CONREC (precision)' if use_conrec else 'scikit-image (standard)'}")  # NEW
    ...
    analysis_params = {
        ...
        'use_conrec': use_conrec  # NEW: Pass CONREC flag
    }
    ...
    print(f"Interface extraction method: {'CONREC' if use_conrec else 'scikit-image'}")  # NEW
    ...
    create_parallel_evolution_plots(df, output_dir, resolution, use_conrec)  # NEW: Pass CONREC flag
    
    return df

def create_parallel_evolution_plots(df, output_dir, resolution, use_conrec=False):  # NEW: CONREC parameter
    ...
    method_name = "CONREC" if use_conrec else "scikit-image"  # NEW
    print(f"\n📊 Creating parallel temporal evolution plots ({method_name})...")
    ...
    ax1.set_title(f'Fractal Dimension Evolution ({resolution}×{resolution}) - {method_name}')  # NEW: Include method
    ...
    ax2.set_title(f'Mixing Layer Evolution ({resolution}×{resolution}) - {method_name}')  # NEW: Include method
    ...
    ax3.set_title(f'Fractal Analysis Quality ({resolution}×{resolution}) - {method_name}')  # NEW: Include method
    ...
    ax4.set_title(f'Processing Time Distribution - {method_name}')  # NEW: Include method
    ...
    ax.set_title(f'Fractal Dimension vs Mixing Thickness ({resolution}×{resolution}) - {method_name}')  # NEW: Include method
...
""")
    ...
    df = analyze_parallel_temporal_evolution(
        ...
        args.min_box_size,
        args.processes,
        args.use_conrec  # NEW: Pass CONREC flag
    )
