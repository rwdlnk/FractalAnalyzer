...
def run_parallel_resolution_comparison(data_dirs, resolutions, target_time, output_dir, 
    ...
    use_conrec = analysis_params.get('use_conrec', False)  # NEW
    extraction_method = 'CONREC' if use_conrec else 'scikit-image'  # NEW
...
def create_parallel_comparison_plots(df, output_dir, target_time, use_conrec=False):  # NEW: CONREC parameter
...
""")
    ...
    if args.output_dir is None:
        method_suffix = "_conrec" if args.use_conrec else "_skimage"  # NEW
    ...
    analysis_params = {
        ...
        'use_conrec': args.use_conrec  # NEW: Pass CONREC flag
    }
    ...
    if df is not None and not args.no_plots:
        create_parallel_comparison_plots(df, args.output_dir, args.target_time, args.use_conrec)  # NEW: Pass CONREC flag
