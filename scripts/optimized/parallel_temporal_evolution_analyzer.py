...
def analyze_single_timestep(args):
    ...
    try:
        ...
        # Create analyzer for this specific timestep
        analyzer = RTAnalyzer(timestep_output_dir, use_grid_optimization=False, no_titles=True)
...
def analyze_parallel_temporal_evolution(data_dir, resolution, output_dir=None, 
    ...
    if output_dir is None:
        output_dir = f"./parallel_temporal_evolution_{resolution}x{resolution}"
    ...
    all_vtk_files = sorted(glob.glob(pattern))
    ...
    # Save results
    results_file = os.path.join(output_dir, f'parallel_temporal_evolution_{resolution}x{resolution}.csv')
    ...
    create_parallel_evolution_plots(df, output_dir, resolution)
    
    return df

def create_parallel_evolution_plots(df, output_dir, resolution):
    ...
    print(f"\n📊 Creating parallel temporal evolution plots...")
    ...
    ax1.set_title(f'Fractal Dimension Evolution ({resolution}×{resolution}) - Parallel Analysis')
    ...
    ax2.set_title(f'Mixing Layer Evolution ({resolution}×{resolution}) - Parallel Analysis')
    ...
    ax3.set_title(f'Fractal Analysis Quality ({resolution}×{resolution}) - Parallel Analysis')
    ...
    ax4.set_title(f'Processing Time Distribution - Parallel Analysis')
    ax4.grid(True, alpha=0.7)
    ax4.legend()
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'parallel_temporal_evolution_{resolution}x{resolution}.png'), 
    ...
    ax.set_title(f'Fractal Dimension vs Mixing Thickness ({resolution}×{resolution}) - Parallel Analysis')
    ax.grid(True, alpha=0.7)
    ax.legend()
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'parallel_D_vs_mixing_{resolution}x{resolution}.png'), 
                dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Process all timesteps for 200x200 resolution in parallel
  python parallel_temporal_evolution_analyzer.py --data-dir ~/Research/svofRuns/Dalziel/200x200 --resolution 200

  # Process first 50 timesteps with 6 parallel processes
  ...
  # Analyze specific time range with custom parameters
  python parallel_temporal_evolution_analyzer.py --data-dir ~/Research/svofRuns/Dalziel/200x200 --resolution 200 --start-time 1.0 --end-time 2.0 --processes 4
""")
    ...
    parser.add_argument('--output-dir', default=None,
                       help='Output directory (default: ./parallel_temporal_evolution_RESxRES)')
    ...
    # Run parallel temporal evolution analysis
    ...
    if df is not None:
        print(f"\n🎉 Parallel temporal evolution analysis complete!")
