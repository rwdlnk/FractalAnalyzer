...
def analyze_single_resolution_parallel(args):
    ...
    print(f"🔍 Worker {os.getpid()}: Analyzing {resolution}×{resolution} resolution")
    ...
    # Create unique analyzer for this resolution
    res_output_dir = os.path.join(base_output_dir, f"parallel_resolution_{resolution}x{resolution}")
    analyzer = RTAnalyzer(res_output_dir, use_grid_optimization=False, no_titles=True)
    ...
    try:
        ...
        print(f"✅ Worker {os.getpid()}: {resolution}×{resolution} complete - "
        ...
        print(f"❌ Worker {os.getpid()}: {resolution}×{resolution} failed - {str(e)}")
...
def run_parallel_resolution_comparison(data_dirs, resolutions, target_time, output_dir, 
    ...
    # Save results
    results_file = os.path.join(output_dir, f'parallel_resolution_comparison_t{target_time:.1f}.csv')
...
def create_parallel_comparison_plots(df, output_dir, target_time):
    ...
    print(f"\n📈 Creating parallel comparison plots...")
    ...
    ax1.set_title(f'Fractal Dimension Convergence (t ≈ {target_time}) - Parallel Analysis')
    ...
    ax2.set_title(f'Mixing Layer Convergence (t ≈ {target_time}) - Parallel Analysis')
    ...
    ax3.set_title(f'Interface Complexity vs Resolution (t ≈ {target_time}) - Parallel Analysis')
    ...
    ax4.set_title(f'Analysis Quality & Performance (t ≈ {target_time}) - Parallel Analysis')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'parallel_resolution_comparison_t{target_time:.1f}.png'), 
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Compare 200x200 and 400x400 at t=2.0 in parallel
  ...
  # Compare multiple resolutions with custom process count
  python parallel_resolution_comparison.py \\
...
""")
    ...
    parser.add_argument('--output-dir', default=None,
                       help='Output directory (default: ./parallel_resolution_comparison_tX.X)')
    ...
    if args.output_dir is None:
        args.output_dir = f"./parallel_resolution_comparison_t{args.target_time:.1f}"
    ...
    if df is not None and not args.no_plots:
        create_parallel_comparison_plots(df, args.output_dir, args.target_time)
